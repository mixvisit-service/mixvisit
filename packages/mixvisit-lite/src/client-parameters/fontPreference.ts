 ...
 */
export function getFontPreferences(): Promise<Record<string, number>> {
...
function withNaturalFonts<T>(
  ...
  // iframe: HTMLIFrameElement, iWindow: typeof window

  const actionProp = (_: HTMLIFrameElement, iframeWindow: typeof window) => {
  ...
  const initialHtml = '<!doctype html><html><head><meta name="viewport" content="width=device-width, initial-scale=1">';
